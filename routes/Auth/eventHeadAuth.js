...
router.get("/logout", authSuperUser, function (req, res, next) {
  ...
  logger.log(
    "info",
    `Head User Logging Out ${JSON.stringify(req.session.data)}`
  );
  req.session.headUserData = null;
  req.session.isHeadUser = false;
  req.session.isHeadUserLogged = false;
  req.session.destroy(function (err) {
    if (err) {
      logger.log("error", `Head User Logout Error ${JSON.stringify(err)}`);
      req.flash("error", "Oops! Something Happened! Unable to Logout!");
      return res.redirect("/event/head/home");
    } else {
      logger.log("info", `HEAD User Logged Out Successfully`);
...
router.get("/reset/:token", (req, res) => {
  try {
    ...
    connection.query(sql, [req.params.token], (err, data) => {
      if (err) {
        ...
        if (data.length > 0) {
          ...
          logger.log(
            "info",
            `HEAD User Reset Password Page Rendered ${JSON.stringify(
              req.params.token
            )}`
          );
    ...
    logger.log(
      "error",
      `Head User Reset Password Page Catch Block Error ${JSON.stringify(
        req.params.token
      )}, Error : ${error}`
    );
    return res.redirect("login");
...
router.post("/login", authSuperUserCheck, (req, res) => {
  if (req.body.email && req.body.password) {
    ...
    connection.query(sql, [req.body.email], async (err, data) => {
      if (err) {
        ...
        if (data.length > 0) {
          ...
          if (validPass) {
            req.session.headUserData = data;
            req.session.isHeadUser = true;
            req.session.isHeadUserLogged = true;
            logger.log("info", `${req.body.email} Head User Logged IN`);
            ...
            logger.log(
              "error",
              `Head User Login Password MissMatch Error ${JSON.stringify(
                req.body
              )}`
            );
            return res.redirect("login");
    ...
    logger.log(
      "error",
      ` Head User Login No username or password ${JSON.stringify(req.body)}`
    );
...
router.post("/reset-password", (req, res) => {
  try {
    if (req.body.email === undefined) {
      ...
      let SuperSql = "SELECT id from headuser where email = ?";
      connection.query(SuperSql, [req.body.email], (errSuper, dataSuper) => {
        if (errSuper) {
          logger.log(
            "error",
            `Head User Forgot Password EMAIL Error ${JSON.stringify(
              req.body
            )}, Error : ${errSuper}`
          );
          ...
          if (dataSuper.length < 1) {
            ...
            crypto.randomBytes(20, (err, buf) => {
              if (err) {
                logger.log(
                  "error",
                  `Head User Forgot Password CRYPTO Error ${JSON.stringify(
                    req.body
                  )}, Error : ${err}`
                );
                ...
                connection.query(
                  ...
                  (err, data) => {
                    if (err) {
                      logger.log(
                        "error",
                        `Head User Forgot Password Write DB Error ${JSON.stringify(
                          req.body
                        )}, Error : ${err}`
                      );
                      return res.status(400).json({
                      ...
                      sendHeadMail(
                        ...
                        if (mailerr) {
                          ...
                          logger.log(
                            "info",
                            `Head User Forgot Password ${JSON.stringify(
                              req.body.email
    ...
    logger.log(
      "error",
      `Head User Forgot Password Catch Block Error ${JSON.stringify(
        req.body
      )}, Error : ${error}`
    );
    return res.status(400).json({
      message: `Head User Forgot Password Catch Block Error ${JSON.stringify(
        req.body
      )}`,
      err: error,
    });
...
router.post("/reset", async (req, res) => {
  try {
    ...
    if (error) {
      ...
      logger.log(
        "error",
        `Head User Password Reset Schema Error ${JSON.stringify(error)}`
      );
      return res.redirect("/auth/head/reset/" + req.session.resetPasswordToken);
    } else {
      if (req.session.resetPasswordToken && req.session.email) {
        if (req.session.resetPasswordTokenExpires >= Date.now()) {
          let password = await bcrypt.hash(req.body.password, 10);
          let sql = "UPDATE headuser SET password = ? where email = ?";
          connection.query(sql, [password, req.session.email], (err, data) => {
            if (err) {
              ...
              logger.log(
                "error",
                `Head User Reset Password DB Error ${JSON.stringify(
                  req.body
                )}, Error : ${err}`
              );
              return res.redirect(
                "/auth/head/reset/" + req.session.resetPasswordToken
              );
            } else {
              req.flash("success", "Your Password  is reset Successfully!");
              logger.log(
                "info",
                `Head User Reset Password Done ${JSON.stringify(
                  req.body
              ...
              connection.query(
                ...
                (errSub, dataSub) => {
                  if (errSub) {
                    logger.log(
                      "error",
                      `Head User Reset Password REMOVE TOKEN DB Error ${req.session.resetPasswordToken}, Error : ${errSub}`
                    );
                  } else {
                    logger.log(
                      "info",
                      `Head User Reset Passsword REMOVED TOKEN SUCCSES ${req.session.resetPasswordToken}`
                    );
                  }
        ...
        logger.log(
          "error",
          `Head User Reset Password SESSION Block Error ${JSON.stringify(
            req.session
    ...
    logger.log(
      "error",
      `Head User Reset Password Catch Block Error ${JSON.stringify(
        req.body
      )}, Error : ${error}`
    );
    return res.redirect("/auth/head/reset/" + req.session.resetPasswordToken);
